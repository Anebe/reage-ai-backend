...
//        val response = conn.inputStream.bufferedReader().readText()
    ...
    fun getAccessToken(): String {
        if(!EfiClient.isExpire()){
            return EfiClient.token
        }
        ...
        if (EfiClient.token.isBlank()){
            EfiClient.token = token
            val expireInSeconds = map["expires_in"] as Int
            EfiClient.expiresIn = LocalDateTime.now().plusSeconds(expireInSeconds.toLong())
        }
        return token
    }
